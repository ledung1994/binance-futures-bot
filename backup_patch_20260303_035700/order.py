...
def place_market_order(symbol, side, quantity, min_notional_usdt=20.0):
    ...
    try:
        ...
        trade_data = {
            ...
            "side": side,
            "quantity": qty,
            "entry_price": float(order.get("avgPrice") or (float(order.get("cumQuote", 0))/float(order.get("executedQty", 1)) if float(order.get("executedQty", 0)) > 0 else price)),
...
client = get_client()
return client.futures_create_order(symbol=symbol, side=sl_side, type="STOP_MARKET", stopPrice=str(round(stop_price,4)), closePosition="true")
except Exception as e:
pass
...
client = get_client()
return client.futures_create_order(symbol=symbol, side=tp_side, type="TAKE_PROFIT_MARKET", stopPrice=str(round(tp_price,4)), closePosition="true")
except Exception as e:
pass
